			...
			switch (tag >>> 3) {
				...
				case 3:
					message.issuance.push(
						MsgCreateBatchRequest_BatchIssuance.decode(
							reader,
							reader.uint32()
						)
					);
...
export const MsgCreateBatchRequest_BatchIssuance = {
	$type: 'regen.ecocredit.v1alpha1.MsgCreateBatchRequest.BatchIssuance' as const,
...
export const MsgCreateBatchResponse = {
	...
	fromJSON(object: any): MsgCreateBatchResponse {
		return {
			...
			batchDenom: isSet(object.batchDenom)
...
export const MsgSendRequest = {
	...
	encode(
		...
		for (const v of message.credits) {
			MsgSendRequest_SendUnits.encode(
				v!,
				writer.uint32(26).fork()
			).ldelim();
		}
		...
		return {
			...
			credits: Array.isArray(object?.credits)
				? object.credits.map((e: any) =>
						MsgSendRequest_SendUnits.fromJSON(e)
				  )
				: [],
		};
	},
	...
	fromPartial<I extends Exact<DeepPartial<MsgSendRequest>, I>>(
		...
		message.credits =
			object.credits?.map((e) =>
				MsgSendRequest_SendUnits.fromPartial(e)
			) || [];
		return message;
	},
};
...
export const MsgRetireRequest = {
	...
	decode(input: _m0.Reader | Uint8Array, length?: number): MsgRetireRequest {
		...
		while (reader.pos < end) {
			...
			switch (tag >>> 3) {
				...
				case 2:
					message.credits.push(
						MsgRetireRequest_RetireUnits.decode(
							reader,
							reader.uint32()
						)
					);
	...
	fromPartial<I extends Exact<DeepPartial<MsgRetireRequest>, I>>(
		...
		message.credits =
			object.credits?.map((e) =>
				MsgRetireRequest_RetireUnits.fromPartial(e)
			) || [];
		return message;
	},
};
